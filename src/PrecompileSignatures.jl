...
function _split_unions(sig::DataType)::Set{Tuple}
    ...
    unpacked = map(_unpack_union!, types)
    # @show unpacked
    # for unpacked_types in unpacked
    #    if !(all(isconcretetype, unpacked_types))
    #        return Set{Tuple}()
    #    end
    # end
    pairs = _pairs(unpacked)
...
function precompilables(M::Vector{Module}, config::Config=Config())::Vector{DataType}
    ...
    types = map(M) do mod
        ...
        directives = [_directives_datatypes(sig, config.split_unions) for sig in signatures]
